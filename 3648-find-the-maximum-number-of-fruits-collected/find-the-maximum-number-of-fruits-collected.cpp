...
public:
    int maxCollectedFruits(vector<vector<int>>& fruits) {
        ...
        //Collect fruits on the main diagonal
        ll first = 0;
        for (int i = 0; i < n; ++i)
            first += fruits[i][i];

        //Second path (from top-right): only down-left/down/down-right
        vector<pair<int,int>> down_dirs = {{1,-1}, {1,0}, {1,1}};
        vector<vector<int>> mem(n, vector<int>(n, -1));
        int second = dfs(fruits, 0, n-1, n-1, mem, down_dirs);

        //Third path (from bottom-left): only up-right/right/down-right
