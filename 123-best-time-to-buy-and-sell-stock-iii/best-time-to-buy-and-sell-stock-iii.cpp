...
public:
    int solve(vector<int> &prices, int i, int buy, int limit, vector<vector<vector<int>>> &dp)
    {
        ...
        return dp[0][1][2]; // answer present from where top down was initiated
    }

    int maxProfit(vector<int> &prices)
    {
        // create 3D dp array --> with 2 rows only
        vector<vector<vector<int>>> dp(2, vector<vector<int>>(2, vector<int>(3, 0))); // limit can be 0,1,2
        return solve(prices, 0, 1, 2, dp);                                           // limit =  2
    }
};